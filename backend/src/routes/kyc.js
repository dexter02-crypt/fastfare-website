...
router.get('/digilocker/auth-url', protect, async (req, res) => {
    try {
        const { DIGILOCKER_CLIENT_ID, DIGILOCKER_REDIRECT_URI } = process.env;
        if (!DIGILOCKER_CLIENT_ID || !DIGILOCKER_REDIRECT_URI) {
            return res.status(500).json({ error: 'DigiLocker credentials not configured' });
        }

        const state = req.user._id.toString(); // Pass user ID in state to link account on callback
        ...
        res.status(500).json({ error: 'Failed to generate DigiLocker auth URL' });
    }
});

// 2. DigiLocker OAuth Callback
router.get('/digilocker/callback', async (req, res) => {
    try {
        ...
        if (error || !code || !state) {
            console.error('DigiLocker callback error/missing params:', error, code, state);
            return res.redirect(`${frontendRedirectBase}/settings?digilocker_status=error`);
        }
        ...
        if (!tokenResponse.ok || !tokenData.access_token) {
            console.error('DigiLocker token exchange error:', tokenData);
            return res.redirect(`${frontendRedirectBase}/settings?digilocker_status=error`);
        }

        // Fetch user data from DigiLocker API
        const userResponse = await fetch('https://digilocker.meripehchaan.gov.in/public/oauth2/1/user', {
            method: 'GET',
            headers: {
        ...
        if (!userResponse.ok) {
            console.error('DigiLocker user fetch error:', digilockerUser);
            return res.redirect(`${frontendRedirectBase}/settings?digilocker_status=error`);
        }

        // Mask Aadhaar: only last 4
        let aadhaarLastFour = null;
        if (digilockerUser.eaadhaar) {
             aadhaarLastFour = digilockerUser.eaadhaar.slice(-4);
        } else if (digilockerUser.uid) {
             aadhaarLastFour = digilockerUser.uid.slice(-4);
        }
        ...
        user.kyc = {
            ...
            digilocker: {
                ...
                dob: digilockerUser.dob || digilockerUser.dateOfBirth || '',
        ...
        return res.redirect(`${frontendRedirectBase}/settings?digilocker_status=success`);
    } catch (err) {
